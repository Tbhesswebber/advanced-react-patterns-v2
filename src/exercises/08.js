...
// @sees https://blog.kentcdodds.com/b40316cfac57
  ...
  internalSetState = (changes, cb) => {
    const newChanges =
      typeof changes === 'function'
        ? changes(this.state, this.props)
        : changes;
    const reducedChanges = this.props.stateReducer(
      this.state,
      newChanges
    );
    if (Object.keys(reducedChanges).length === 0) return;
    return this.setState(reducedChanges, cb);
  };
  reset = () =>
